...
parser.add_argument('--weights_stem', type=str,  default=None,  help='name for the run')
...
parser.add_argument('--default_root_dir', type=str, default='/home/gliomas/brats/models', help='path to output dir')
...
parser.add_argument('--training_batch_size', default=4, type=int, help='Batch size for training')
...
def main(args):
    wandb_logger = WandbLogger(project=args.project_name, group=args.group_name, name = args.weights_stem, reinit=True,            save_dir=os.path.join(args.default_root_dir, args.weights_stem))
    wandb_logger.experiment.config.update(args)
    params = wandb.config
    data = MedicalDecathlonDataModule(params)
    data.prepare_data()
    data.setup()
    ...
    early_stopping = EarlyStopping('val_epoch_loss', patience=params.patience, verbose = True, check_finite= False, min_delta=0.0000)
    checkpoint_callback_loss = ModelCheckpoint(monitor='val_epoch_loss', mode='min', save_top_k=3, filename=os.path.join('{epoch:02d}-{val_loss:.2f}')) 
    checkpoint_callback_dice = ModelCheckpoint(monitor='val_dice', mode='max', save_top_k=3, filename=os.path.join('{epoch:02d}-{val_dice:.2f}')) 
