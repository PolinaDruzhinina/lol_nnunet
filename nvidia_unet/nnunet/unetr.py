...
#         print(self.n_class)
        ...
        if not self.triton:
            ...
            if self.args.brats:
                ...
                self.loss = loss(self.args.focal,  self.args.no_back_in_output)
            if self.args.dim == 2:
                self.tta_flips = [[2], [3], [2, 3]]
            else:
                self.tta_flips = [[2], [3], [4], [2, 3], [2, 4], [3, 4], [2, 3, 4]]
            self.dice = Dice(self.n_class, self.args.brats, self.args.no_back_in_output)
...
#         self.model = UNETR(in_channels=in_channels, out_channels=out_channels, img_size=(128,128,128), pos_embed='conv', norm_name='instance')
#        self.model = UNETR(
